...
function placeXOrO(squareNumber) {
    //This confisiton ensures a square hasnt veen selected already.
    //The .some() method is used to check each element of the selectSquare array
    //to see if it contains the square number clicked on.
    if (!selectedSquares.some(element => element.includes(squareNumber))) {
        //If variable retrieves the HTML element id that was clicked.
        let select = document.getElementById(squareNumber);
        //This condition checks who's turn it is.
        if (activePlayer === 'X') {
            //If activePlayer is equal to 'X', the x.png is placed in HTML
            select.style.backgroundImage = 'url("images/redx.png")';
            //Active player may only be 'X' or 'O' so, if not 'X' it must be 'O'
        } else {
            //If activePlayer is equal to 'O', the o.pgn is placed in HTML
        ...
        if (activePlayer === 'O') {
            //This function disables clikcing for computers turn.
            disableClick();
            //This function waits 1 second before the computer places an image and enables click.
            setTimeout(function () { computersTurn(); }, 1000);
        }
        //Returning true is needed for our comoputersTurn() function to work
        return true;
        }
        //This function results in a ranodm square being xelected by the computer.
...
//This function parses the selectedSquares array to search for win confitions.
//drawLine() function is called to draw a line on the screen if the confition is met.
...
function drawWinLine(coordX1, coordY1, coordX2, coordY2) {
    ...
    let x1 = coordX1,
        ...
        y2 = coordY2,
        //This variable stores temorary x axis data we update in our animation loop.
        x = x1,
    ...
    function animateLineDrawing() {
        ...
        c.stroke();
        //This confition checks if we've reached the endpoints.
        if (x1 <= x2 && y1 <= y2) {
            ...
            //This us necessary for the 6, 4, 2 win conditions.
